...
- (instancetype)initWithFrame:(CGRect)frame viewIdentifier:(int64_t)viewId arguments:(id)args binaryMessenger:(NSObject<FlutterBinaryMessenger> *)messenger {
    ...
    NSString* channelName = [NSString stringWithFormat:@"plugins/flutter_pag_yay.pagview_%lld", viewId];
...
- (void)initPag:(id)arguments result:(FlutterResult _Nonnull)result {
    ...
    if ([assetName isKindOfClass:NSString.class] && assetName.length > 0) {
        ...
        [self pagRenderWithPagData:pagData progress:initProgress repeatCount:repeatCount autoPlay:autoPlay result:result texts:replaceTexts textColors:replaceTextColors imgs:replaceImages];
    }
        ...
        if (!pagData) {
            ...
            [TGFlutterPagDownloadManager download:url completionHandler:^(NSData * _Nonnull data, NSError * _Nonnull error) {
                if (data) {
                    ...
                    [weak_self pagRenderWithPagData:data progress:initProgress repeatCount:repeatCount autoPlay:autoPlay result:result texts:replaceTexts textColors:replaceTextColors imgs:replaceImages];
                }else{
                    result(@-1);
                }
            }];
    ...
    if(bytesData != nil && [bytesData isKindOfClass:FlutterStandardTypedData.class]){
        ...
        if(typedData.type == FlutterStandardDataTypeUInt8 && typedData.data != nil){
            [self pagRenderWithPagData:typedData.data progress:initProgress repeatCount:repeatCount autoPlay:autoPlay result:result texts:replaceTexts textColors:replaceTextColors imgs:replaceImages];
        }else{
            result(@-1);
        }
    }
...
- (void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    NSLog(@"FlutterPagView:%@",call.method);
    ...
    if ([@"getPlatformVersion" isEqualToString:call.method]) {
        ...
        [self changeText:arguments result:result];
    } else {
...
-(void)pagRenderWithPagData:(NSData *)pagData progress:(double)progress repeatCount:(int)repeatCount autoPlay:(BOOL)autoPlay result:(FlutterResult)result texts:(NSArray<NSString *>*)texts textColors:(NSArray<NSString *>*)textColors imgs:(NSArray<NSString *>*)imgs{
...
- (NSArray<NSString *> *)getLayersUnderPoint:(CGPoint)point{
    NSArray<PAGLayer*>* layers = [self.pagView.getComposition getLayersUnderPoint:point];
...
- (void)onAnimationStart:(PAGView* _Nonnull)pagView{
    NSLog(@"FlutterPagView:onAnimationStart %lld",_viewId);
        [_channel invokeMethod:PlayCallback arguments:@{ArgumentTextureId:@(_viewId), ArgumentEvent:EventStart}];
    
}

- (void)onAnimationEnd:(PAGView* _Nonnull)pagView{
    NSLog(@"FlutterPagView:onAnimationEnd %lld",_viewId);
    [_channel invokeMethod:PlayCallback arguments:@{ArgumentTextureId:@(_viewId), ArgumentEvent:EventEnd}];
    
}

- (void)onAnimationCancel:(PAGView* _Nonnull)pagView{
    NSLog(@"FlutterPagView:onAnimationCancel %lld",_viewId);
    [_channel invokeMethod:PlayCallback arguments:@{ArgumentTextureId:@(_viewId), ArgumentEvent:EventCancel}];
    
}


- (void)onAnimationRepeat:(PAGView* _Nonnull)pagView{
//    NSLog(@"FlutterPagView:onAnimationRepeat %lld",_viewId);
    [_channel invokeMethod:PlayCallback arguments:@{ArgumentTextureId:@(_viewId), ArgumentEvent:EventRepeat}];
}

- (void)onAnimationUpdate:(PAGView* _Nonnull)pagView{
